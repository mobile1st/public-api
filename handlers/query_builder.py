...
from handlers.filter_builder import FilterBuilder
...
class QueryBuilder(object):
    ...
    def get_result_from_query(query, page=None, page_size=None):
        ...
        return Page(query.offset(page_size * page).limit(page_size).all(), query.count())
    ...
    def get_result(self, query_filter=None, sorting=None):
        return Page(
            self.get_page(query_filter=query_filter, sorting=sorting), self.get_count(query_filter=query_filter)
        )

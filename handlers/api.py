...
class Api(object):
    ...
    def endpoint(self, path, method):
        ...
        def wrapper(f):
            ...
            def endpoint(*args, **kwargs):
                flask.g.current_language = self.get_language_from_request()
                flask.g.current_ipaddress = self.get_ipaddress_from_country()
                # flask.g.current_country = self.get_country_from_request()
    ...
    def list(self, path, order_by=None, filters=None):
        ...
        ApiDoc.set_wrapper_endpoint(path, "GET")
        ApiDoc.add_wrapper_props({"method": "GET", "list": True, "order_by": order_by, "filters": filters})
    ...
    def upload(self, path, schema=None):
        ApiDoc.set_wrapper_endpoint(path, "POST")
        ApiDoc.add_wrapper_props({"method": "POST", "file_upload": True, "schema": schema})
        validator = None
        if schema:
            validator = Validator(schema, purge_unknown=True)
    ...
    def get_language_from_request(seft):
        lang = "en"
        if flask.request.headers.get(Api.LANGUAGE_HEADER):
            lang = flask.request.headers.get(Api.LANGUAGE_HEADER)

        return lang

    def get_ipaddress_from_country(self):
